...
import HaskellNet.HTTP hiding (request)
...
type Connection = (Handle, String)
instance (BSStream s) => BSStream (s, a) where
    bsGetLine = bsGetLine . fst
    bsGet = bsGet . fst
    bsPut = bsPut . fst
    bsPutNoFlush = bsPutNoFlush . fst
    bsFlush = bsFlush . fst
    bsClose = bsClose . fst
    ...
    where      
        ...
        dorequest hst rqst = 
            do { pool <- getBS bsConnectionPool
               ; let conn = filter (\c -> snd c == uriAuthToString hst) pool
               ; rsp <- case conn of
                    [] -> do { out ("Creating new connection to " ++ uriAuthToString hst)
                             ; let port = case uriPort hst of
                                            (':':s) -> fromIntegral $ read s
                                            _       -> 80
                             ; c <- ioAction $ connectTo (uriRegName hst) (PortNumber port)
                             ; let pool' = if length pool > 5
                                           then init pool
                                           else pool
                             ; when (length pool > 5)
                                    (ioAction $ bsClose (last pool))
                             ; alterBS (\b -> b { bsConnectionPool=(c, uriAuthToString hst):pool' })
